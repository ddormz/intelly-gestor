...
const schema = z.object({
  ...
  tenantApiKey: z.string().trim().max(500).optional(),
  systemApiKey: z.string().trim().max(500).optional(),
  tenantRut: z.string().trim().max(20).optional(),
...
export async function testWebpayConfigAction(_: ActionState, _formData: FormData): Promise<ActionState> {
  try {
    await enforceSameOrigin();
    await requireUser("admin");
    const result = await testWebpayConnection();
    return { status: result.ok ? "success" : "error", message: result.safeMessage };
  } catch (error) {
    return { status: "error", message: safeError(error).message };
  }
...
export async function testSmtpConfigAction(_: ActionState, _formData: FormData): Promise<ActionState> {
  try {
    await enforceSameOrigin();
    await requireUser("admin");
...
export async function testDatabaseAction(_: ActionState, _formData: FormData): Promise<ActionState> {
  try {
    await enforceSameOrigin();
    await requireUser("admin");
    const start = Date.now();
    const ok = await databaseHealth();
    const duration = Date.now() - start;
    return {
      status: ok ? "success" : "error",
      message: ok ? `Conexión a MySQL activa (${duration}ms)` : "Fallo de conexión a la base de datos MySQL",
    };
  } catch (error) {
    return { status: "error", message: safeError(error).message };
  }
