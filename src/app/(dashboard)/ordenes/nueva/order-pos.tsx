...
type PosLine = PosDraftLine & { rowKey: string; code: string; taxRate: number; taxCategory: "taxable" | "exempt"; rawUnitPrice?: string };
...
export type OrderPosInitial = {
  ...
  dueAt?: string | null;
  notes?: string | null;
  lines?: Array<{ id?: string; catalogItemId: string | null; code: string | null; description: string; quantity: number; unitPrice: number; taxRate: number }>;
};
...
export function OrderPos({ action, initial }: { action: (state: ActionState, formData: FormData) => Promise<ActionState>; initial?: OrderPosInitial }) {
  ...
  const [lines, setLines] = useState<PosLine[]>(() => (initial?.lines ?? []).map((line, index) => ({
  ...
  const calculated = lines.length ? calculateOrder(lines.map((line) => ({ description: line.name?.trim() || line.description?.trim() || "Ítem libre", quantity: line.quantity, unitPrice: line.unitPrice, taxRate: line.taxRate, taxCategory: line.taxCategory })), discountPercent, discountReason || "Descuento") : null;
  ...
  function addCatalog(item: CatalogResult) {
    setLines((current) => {
      ...
      return [...current, { rowKey: item.id, catalogItemId: item.id, code: item.code, name: item.name, description: "", quantity: 1, unitPrice: Number(item.unitPrice), taxRate: Number(item.taxRate), taxCategory: item.taxCategory }];
    });
    setCatalogQuery("");
    setCatalogResults([]);
  }
