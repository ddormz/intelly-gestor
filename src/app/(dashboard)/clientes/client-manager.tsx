...
import { Download, FileDown, Pencil, Power, Upload, UserRoundPlus } from "lucide-react";
...
function ClientFields({ item, errors }: { item?: ClientListItem; errors?: Record<string, string[]> }) {
  return <>
    ...
    <Field label="RUT" error={errors?.taxId?.[0]}><Input required name="taxId" defaultValue={item?.taxId ?? ""} placeholder="76.123.456-0" /></Field>
...
export function ClientManager({ items, canImport }: { items: ClientListItem[]; canImport: boolean }) {
  ...
  const actions = <>{create}{canImport ? <ActionModal triggerLabel="Importar" triggerIcon={<Upload size={18} />} variant="secondary" title="Importar clientes" description="Usa la plantilla CSV. El archivo completo se valida antes de guardar." submitLabel="Importar archivo" pendingLabel="Importando…" action={importClientsAction}>{() => <Field label="Archivo CSV" hint="Máximo 2 MiB y 5.000 filas."><Input required name="file" type="file" accept=".csv,text/csv" /></Field>}</ActionModal> : null}<Link className="btn-secondary" href="/api/export/clientes"><Download size={18} />Exportar</Link><Link className="btn-secondary" href="/api/export/clientes?template=1"><FileDown size={18} />Plantilla</Link></>;
